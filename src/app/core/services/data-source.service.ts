...
export class DataSourceService {
    ...
    getDataSources(offset: number = 0, limit: number = 10): Observable<PagedResponse<DataSource>> {
        return this.http.get<PagedResponse<DataSource>>(`${environment.apiUrl}/datasources`, { params: { offset, limit } });
    }

    getDataSource(id: number): Observable<DataSource> {
        return this.http.get<DataSource>(`${`${environment.apiUrl}/datasources`}/${id}`);
    }
}
